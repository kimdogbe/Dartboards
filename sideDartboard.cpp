...
void sobel(Mat src);
void hough(Mat imgMag, Mat imgGrad, int minRad, int maxRad);
...
int main( int argc, char** argv )
{
	...
	// LOADING THE IMAGE
	 ...
	 cvtColor(image, gray_image, CV_BGR2GRAY); // not using blurrd image btw
...
void sobel(Mat src){
  ...
  Mat kerGRY = (Mat_<float>(3,3) << 1, 2, 1,
   ...
   Mat kernel = (Mat_<float>(3,3) << 1, 1, 1,
     ...
     imageOrientation.at<float>(y,x) = (float)atan2( imageDY.at<float>(y,x), imageDX.at<float>(y,x) ) * 180/CV_PI;

		 //set threshold for image magnitude
     if(imageMagnitude.at<float>(y,x) > 200)
     {
  ...
  //normalize(imageOrientation, imageOrientation, 0, 255, CV_MINMAX);
	...
	hough(imageMagnitude, imageOrientation, 10, 60);
}
